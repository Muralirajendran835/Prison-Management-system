<?php

// Database Connection

$host="localhost";
$uname="root";
$pass="";


$connection=mysqli_connect($host,$uname,$pass); 

echo mysqli_error($connection);

//or die("Database Connection Failed");
$selectdb=mysqli_select_db($connection,"prisonpro") or die("Database could not be selected");	
$result=mysqli_select_db($connection,"prisonpro")or die("database cannot be selected <br>");

	
// Fetch Record from Database

$output			= "";
$table 			= "registration"; // Enter Your Table Name
$sql 			= mysqli_query($connection,"select * from $table");
$columns_total 	= mysqli_num_fields($sql);

// Get The Field Name

for ($i = 0; $i < $columns_total; $i++) {
	$heading	=	mysqli_fetch_field_direct($sql, $i);
	$output		.= '"'.$heading->name.'",';
}
$output .="\n";

// Get Records from the table

while ($row = mysqli_fetch_array($sql)) {
for ($i = 0; $i < $columns_total; $i++) {
$output .='"'.$row["$i"].'",';
}
$output .="\n";
}

// Download the file

$filename =  "prisoner_report.csv";
header('Content-type: application/csv');
header('Content-Disposition: attachment; filename='.$filename);

echo $output;
exit;
	
?>
